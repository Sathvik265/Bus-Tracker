...
const LoginModal: React.FC<LoginModalProps> = ({ onLogin, onClose }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-40 flex justify-center items-center animate-fade-in">
      <div className="bg-white rounded-lg shadow-2xl p-8 m-4 max-w-sm w-full relative transform transition-all duration-300 scale-100">
        <button 
          onClick={onClose}
        ...
        <div className="text-center">
            ...
            <p className="mt-2 text-slate-600">Please login to continue to your bookings.</p>
        </div>
        <div className="mt-8">
            <button
                onClick={onLogin}
                className="w-full flex items-center justify-center bg-white border border-slate-300 text-slate-700 font-semibold py-3 px-4 rounded-lg hover:bg-slate-50 transition-colors shadow-sm"
            >
                <GoogleIcon />
                Login with Google
            </button>
